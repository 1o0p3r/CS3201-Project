...
tuple<bool, vector<vector<string>>> NextAnalyzer::addArgTwoResult(string arg1)
{
	...
	for (int candidates : vecOfCandidates) {
		pkbNext = pkbReadOnly.getPrevious(candidates);
...
{
	auto pkbResult = pkbReadOnly.getPrevious(stoi(arg1));
...
{
	return pkbReadOnly.getPrevious(stoi(arg1)).empty() ? false : true;
}

bool NextAnalyzer::checkClauseWildVariable(string arg2)
{
	return pkbReadOnly.getNext(stoi(arg2)).empty() ? false : true;
}
